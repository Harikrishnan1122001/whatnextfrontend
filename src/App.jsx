...
// // NEW: Flip-in from top
// function RevealFlip({ children, delay = 0, className = "", style = {} }) {
//   const [ref, visible] = useScrollReveal({ threshold: 0.1 });
//   return (
//     <div ref={ref} className={className} style={{
//       opacity: visible ? 1 : 0,
//       transform: visible ? "perspective(600px) rotateX(0deg)" : "perspective(600px) rotateX(-20deg)",
...
// // NEW: Typewriter animated number counter
...
//   };
//   const handleLeave = () => {
//     if (ref.current) ref.current.style.transform = "perspective(600px) rotateY(0deg) rotateX(0deg) translateY(0) scale(1)";
//   };
//   return (
//     <div
//       ref={ref}
...
//   return (
//     <div
//       className={`mascot-card ${active ? "active" : ""}`}
//       onClick={onClick}
...
//   /* NEW: particle burst on mascot */
//   @keyframes particleBurst {
//     0%   { transform: translate(-50%,-50%) scale(1); opacity: 1; }
//     100% {
//       transform: translate(
//         calc(-50% + calc(cos(var(--angle, 0deg)) * var(--dist, 30px))),
//         calc(-50% + calc(sin(var(--angle, 0deg)) * var(--dist, 30px)))
//       ) scale(0);
//       opacity: 0;
//     }
//   }

//   /* NEW: border trace animation on cards */
...
//   /* NEW: text typewriter cursor blink */
//   @keyframes cursorBlink {
//     0%, 100% { opacity: 1; }
//     50%       { opacity: 0; }
//   }

//   /* NEW: live dot expanded pulse */
//   @keyframes livePulseRing {
//     0%   { box-shadow: 0 0 0 0 rgba(232,17,59,0.5); }
//     70%  { box-shadow: 0 0 0 8px rgba(232,17,59,0); }
//     100% { box-shadow: 0 0 0 0 rgba(232,17,59,0); }
//   }

//   /* NEW: glitch keyframes */
//   @keyframes glitch1 {
//     0%   { clip-path: inset(20% 0 60% 0); transform: translate(-4px); }
//     50%  { clip-path: inset(60% 0 10% 0); transform: translate(4px); }
//     100% { clip-path: inset(40% 0 40% 0); transform: translate(0); }
//   }
//   @keyframes glitch2 {
//     0%   { clip-path: inset(50% 0 20% 0); transform: translate(4px); color: var(--accent); }
//     50%  { clip-path: inset(10% 0 60% 0); transform: translate(-4px); color: var(--cyan); }
//     100% { clip-path: inset(30% 0 50% 0); transform: translate(0); }
//   }

//   /* NEW: shine sweep on buttons */
...
//     0%, 100% { box-shadow: 0 0 0 0 rgba(232,17,59,0.4); }
//     50%       { box-shadow: 0 0 0 6px rgba(232,17,59,0); }
//   }
...
//   /* NEW: section eyebrow animated underline */
//   .section-eyebrow {
//     font-size: 0.8rem;
//     font-weight: 700;
//     letter-spacing: 2px;
...
//   /* NEW: scroll progress bar under navbar */
//   .scroll-progress {
...
//   /* NEW: nav link with animated bottom border */
//   .nav-link {
//     padding: 8px 16px; border-radius: var(--radius-sm); color: var(--text2);
...
//   /* NEW: avatar with ring animation */
//   .avatar-btn {
...
//   /* NEW: shine sweep on btn-primary hover */
...
//   /* NEW: Enhanced cards with gradient border trace on hover */
//   .card {
...
//     border-radius: var(--radius);
//     padding: 1px;
//     background: linear-gradient(135deg, transparent 40%, rgba(232,17,59,0.4), transparent 60%);
...
//     box-shadow: var(--shadow), 0 0 0 1px var(--primary-glow), 0 20px 40px rgba(232,17,59,0.14);
//   }
...
//   /* NEW: animated mesh gradient blobs in hero */
//   .hero::before {
//     content: ''; position: absolute; top: -200px; right: -200px; width: 600px; height: 600px;
//     background: radial-gradient(circle, rgba(232,17,59,0.10) 0%, transparent 70%);
//     pointer-events: none; animation: floatYSlow 8s ease-in-out infinite;
//   }
//   .hero::after {
...
//     pointer-events: none; animation: floatYSlow 11s ease-in-out infinite reverse;
//   }

//   .hero-eyebrow {
//     display: inline-flex; align-items: center; gap: 8px;
//     background: var(--bg3); border: 1px solid var(--border); border-radius: 20px;
//     padding: 6px 16px; font-size: 0.82rem; color: var(--primary); margin-bottom: 28px; font-weight: 600;
//   }
...
//   /* Loading — enhanced */
//   .spinner { width: 40px; height: 40px; border: 3px solid var(--border); border-top-color: var(--primary); border-radius: 50%; animation: spin 0.7s cubic-bezier(0.68,-0.55,0.27,1.55) infinite; margin: 60px auto; display: block; }
...
//   tr { transition: background 0.15s, transform 0.15s; }
...
//   .search-bar { display: flex; align-items: center; gap: 10px; background: #fff; border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 10px 16px; margin-bottom: 28px; transition: border-color 0.2s, box-shadow 0.2s, transform 0.2s; }
//   .search-bar:focus-within { border-color: var(--primary); box-shadow: 0 0 0 3px var(--primary-glow); transform: scale(1.005); }
//   .search-bar input { flex: 1; background: none; border: none; color: var(--text); font-size: 0.9rem; outline: none; }
//   .search-bar input::placeholder { color: var(--text3); }

//   /* Category strip — enhanced mascot cards */
...
//   }
//   .glitch-active::before { animation: glitch1 0.3s steps(2) both; color: var(--cyan); }
//   .glitch-active::after  { animation: glitch2 0.3s steps(2) both; color: var(--accent); }
...
//   /* Video manager */
//   .video-manager-item { display: flex; align-items: center; gap: 12px; padding: 12px; background: var(--bg2); border: 1px solid var(--border); border-radius: var(--radius-sm); margin-bottom: 8px; transition: all 0.25s cubic-bezier(0.22,1,0.36,1); }
...
//       <img src={LOGO_URL} alt="WhatNext" className="logo-img" />
//     </div>
//   );
// }
...
//             <input className="form-input" placeholder="Your name" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} />
//           </div>
//         )}
//         <div className="form-group">
...
//         <MagneticBtn className="btn btn-primary btn-lg" style={{ width: "100%", justifyContent: "center" }} onClick={submit} disabled={loading}>
...
// // ─────────────────────────────────────────────
// // CATEGORY DATA & STRIP (with particle effect)
// // ─────────────────────────────────────────────
// const COURSE_CATS = [
...
//       {COURSE_CATS.map(a => (
//         <ParticleMascotCard key={a.level + a.name} emoji={a.emoji} name={a.name} label={a.label} active={selected === a.level} onClick={() => onSelect(a.level)} />
//       ))}
//     </div>
//   );
// }
...
//           : <div style={{ textAlign: "center" }}><div style={{ fontSize: "3rem" }}>📡</div><div style={{ fontSize: "0.75rem", color: "var(--text3)", marginTop: 8 }}>Live Class</div></div>
//         }
...
//           : <div style={{ textAlign: "center" }}><div style={{ fontSize: "3rem" }}>📄</div><div style={{ fontSize: "0.75rem", color: "var(--text3)", marginTop: 8 }}>{note.fileType?.toUpperCase() || "PDF"}</div></div>
//         }
//       </div>
//       <div className="card-body">
//         <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
...
//             {user
//               ? <MagneticBtn className="btn btn-primary btn-lg" onClick={() => navigate("/courses")}>Browse Courses 🚀</MagneticBtn>
//               : <MagneticBtn className="btn btn-primary btn-lg" onClick={onAuth}>Get Started Free →</MagneticBtn>
//             }
...
//                   <AnimatedNumber target={parseFloat(n)} suffix={sfx} duration={1400} />
//                 </span>
...
//               <RevealBounce key={title} delay={i * 90}>
...
//                     <RevealBounce key={c._id} delay={i * 80}>
...
//                 <RevealScale key={lc._id} delay={i * 90}>
...
//               </p>
//               <MagneticBtn className="btn btn-lg" style={{ background: "#fff", color: "var(--primary)" }} onClick={user ? () => navigate("/courses") : onAuth}>
...
//               <div className="grid-3">
//                 {courses.map((c, i) => (
//                   <RevealBounce key={c._id} delay={i * 60}>
//                     <CourseCard course={c} onClick={() => navigate(`/courses/${c._id}`)} />
//                   </RevealBounce>
//                 ))}
//               </div>
...
//         handler: async (res) => {
//           const verify = await authFetch("/payments/verify", { method: "POST", body: JSON.stringify({ razorpayOrderId: res.razorpay_order_id, razorpayPaymentId: res.razorpay_payment_id, razorpaySignature: res.razorpay_signature, itemType: "course", itemId: id }) });
...
//                         <div style={{ display: "flex", gap: 8, fontSize: "0.87rem", color: "var(--text2)" }}>
...
//               {hasPurchased
//                 ? <MagneticBtn className="btn btn-green btn-lg sidebar-btn" onClick={() => navigate("/dashboard")}>Go to My Learning →</MagneticBtn>
//                 : <MagneticBtn className="btn btn-primary btn-lg sidebar-btn" onClick={handleBuy} disabled={paying}>
...
//       .then(r => r.json())
//       .then(d => { if (d.success) setClasses(d.data || d.liveClasses || d.classes || []); setLoading(false); })
//       .catch(() => setLoading(false));
//   }, [status]);
...
//               <div className="grid-3">
//                 {classes.map((lc, i) => (
//                   <RevealBounce key={lc._id} delay={i * 70}>
//                     <LiveClassCard liveClass={lc} onClick={() => navigate(`/live/${lc._id}`)} />
//                   </RevealBounce>
//                 ))}
//               </div>
...
//         const options = {
//           key: order.razorpayKeyId || '',
//           amount: order.order?.amount, currency: order.order?.currency || 'INR',
//           name: 'WhatNext', description: lc.title, order_id: order.order?.id,
//           handler: async (razorpayRes) => {
//             const verify = await authFetch('/payments/verify', { method: 'POST', body: JSON.stringify({ razorpayOrderId: razorpayRes.razorpay_order_id, razorpayPaymentId: razorpayRes.razorpay_payment_id, razorpaySignature: razorpayRes.razorpay_signature, itemType: 'liveClass', itemId: id }) });
//             if (verify.success) { toast("Payment successful! You're registered 🎉", 'success'); const headers = token ? { Authorization: `Bearer ${token}` } : {}; fetch(`${API_BASE}/live-classes/${id}`, { headers }).then(r => r.json()).then(d => { if (d.success) setLc(d.data || d.liveClass); }); }
//             else toast(verify.message || 'Payment verification failed', 'error');
//           },
//           prefill: { name: user.name, email: user.email },
...
//                     <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid var(--border)", fontSize: "0.87rem" }}>
...
//                   : <MagneticBtn className="btn btn-primary btn-lg sidebar-btn" onClick={handleRegister} disabled={registering || lc.isFull}>
...
//               <div className="grid-3">
//                 {notes.map((n, i) => (
//                   <RevealBounce key={n._id} delay={i * 65}>
...
//         key: order.razorpayKeyId || "",
//         amount: order.order?.amount, currency: order.order?.currency || "INR",
//         name: "WhatNext", description: note.title, order_id: order.order?.id,
//         handler: async (res) => {
//           const verify = await authFetch("/payments/verify", { method: "POST", body: JSON.stringify({ razorpayOrderId: res.razorpay_order_id, razorpayPaymentId: res.razorpay_payment_id, razorpaySignature: res.razorpay_signature, itemType: "notes", itemId: id }) });
...
//                 ? <MagneticBtn className="btn btn-green btn-lg sidebar-btn" onClick={handleDownload}>⬇️ Download Now</MagneticBtn>
//                 : <MagneticBtn className="btn btn-primary btn-lg sidebar-btn" onClick={handleBuy} disabled={paying}>
...
//             <RevealBounce key={label} delay={i * 80}>
//               <div className="stat-card">
//                 <div className="stat-card-icon">{icon}</div>
//                 <div className="stat-card-num"><AnimatedNumber target={num} duration={800} /></div>
...
//             : <div className="grid-3">
//                 {courses.map((p, i) => (
//                   <RevealBounce key={p._id || p.course} delay={i * 70}>
...
//             : <div className="grid-3">
//                 {notes.map((p, i) => (
//                   <RevealBounce key={p._id || p.notes} delay={i * 70}>
//                     <div className="card">
//                       <div className="card-body">
...
// // ═══════════════════════════════════════════════════════════════════
// // ADMIN PANEL (same as original, keeping all modals intact)
// // ═══════════════════════════════════════════════════════════════════
...
//     xhr.onload = () => {
//       setSaving(false);
//       try { const res = JSON.parse(xhr.responseText); if (res.success) { toast(`Notes ${isEdit ? "updated" : "uploaded"}! ✅`, "success"); onSave(); onClose(); } else toast(res.message || "Failed", "error"); }
...
//                   {[["👥", stats.users?.total ?? 0, "Total Users"], ["📚", stats.courses?.total ?? 0, "Courses"], ["📡", stats.liveClasses?.total ?? 0, "Live Classes"], ["📄", stats.notes?.total ?? 0, "Notes"], ["💳", stats.revenue?.transactions ?? 0, "Payments"], ["💰", `₹${(stats.revenue?.total ?? 0).toLocaleString()}`, "Revenue"]].map(([icon,num,label],i) => (
//                     <RevealBounce key={label} delay={i*70}>
//                       <div className="stat-card">
//                         <div className="stat-card-icon">{icon}</div>
//                         <div className="stat-card-num">{typeof num === "number" ? <AnimatedNumber target={num} duration={900} /> : num}</div>
//                         <div className="stat-card-label">{label}</div>
//                       </div>
//                     </RevealBounce>
//                   ))}
...
//                     <MagneticBtn className="btn btn-primary" onClick={() => { setCourseFormData({}); loadSection("courses"); }}>+ New Course</MagneticBtn>
//                     <MagneticBtn className="btn btn-outline" onClick={() => { setLiveFormData({}); loadSection("live"); }}>+ New Live Class</MagneticBtn>
//                     <MagneticBtn className="btn btn-outline" onClick={() => { setNotesFormData({}); loadSection("notes"); }}>+ Upload Notes</MagneticBtn>
//                     <MagneticBtn className="btn btn-outline" onClick={() => loadSection("users")}>View Users</MagneticBtn>
//                     <MagneticBtn className="btn btn-outline" onClick={() => loadSection("payments")}>View Payments</MagneticBtn>
//                   </div>
//                 </div>
//               </Reveal>
//             </>
...
//                         </tr>
//                       ))}
//                       {liveClasses.length === 0 && <tr><td colSpan={7} style={{ textAlign: "center", padding: 40, color: "var(--text3)" }}>No live classes yet.</td></tr>}
//                     </tbody>
//                   </table>
//                 </div>
//               </Reveal>
...
//                         </tr>
//                       ))}
//                       {notes.length === 0 && <tr><td colSpan={7} style={{ textAlign: "center", padding: 40, color: "var(--text3)" }}>No notes yet.</td></tr>}
//                     </tbody>
//                   </table>
//                 </div>
//               </Reveal>
...
//             <>
//               <Reveal>
//                 <div className="admin-header"><div className="admin-title">Users</div><div className="admin-sub">{users.length} registered users</div></div>
//               </Reveal>
//               <Reveal delay={80}>
//                 <div className="table-wrap">
//                   <table>
...
//             <>
//               <Reveal>
//                 <div className="admin-header"><div className="admin-title">All Payments</div><div className="admin-sub">Complete transaction history</div></div>
//               </Reveal>
//               <Reveal delay={80}>
//                 <div className="table-wrap">
//                   <table>
...
//             <div>
//               <div style={{ marginBottom: 14 }}><Logo onClick={() => navigate("/")} /></div>
...
//                 {links.map(([label, to]) => (
//                   <div key={label} style={{ fontSize: "0.82rem", color: "var(--text3)", marginBottom: 8, cursor: "pointer", transition: "color 0.2s, transform 0.2s", display: "inline-block" }}
//                     onClick={() => navigate(to)}
//                     onMouseEnter={e => { e.target.style.color = "var(--primary)"; e.target.style.transform = "translateX(4px)"; }}
//                     onMouseLeave={e => { e.target.style.color = "var(--text3)"; e.target.style.transform = "translateX(0)"; }}>
//                     {label}
//                   </div>
//                 ))}
//               </div>
...
//           <div style={{ fontSize: "5rem", marginBottom: 20, animation: "floatYSlow 3s ease-in-out infinite" }}>💻</div>
//           <h2 style={{ fontFamily: "var(--font2)", marginBottom: 12 }}>404 — Page not found</h2>
//           <MagneticBtn className="btn btn-primary" onClick={() => navigate("/")}>Go Home</MagneticBtn>
//         </div>
//       </div>
//     );
//   };
...
// Typewriter animated number counter
function AnimatedNumber({ target, prefix = "", suffix = "", duration = 1200 }) {
  const [count, setCount] = useState(0);
  const [ref, visible] = useScrollReveal({ threshold: 0.5 });
  const started = useRef(false);
...
// Particle burst on hover for mascot cards
function ParticleMascotCard({ emoji, name, label, active, onClick }) {
  const [particles, setParticles] = useState([]);
  const handleEnter = () => {
    const p = Array.from({ length: 8 }, (_, i) => ({
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* text typewriter cursor blink */
  @keyframes cursorBlink {
    0%, 100% { opacity: 1; }
    50%       { opacity: 0; }
  }

  /* live dot expanded pulse */
  @keyframes livePulseRing {
    0%   { box-shadow: 0 0 0 0 rgba(232,17,59,0.5); }
    70%  { box-shadow: 0 0 0 8px rgba(232,17,59,0); }
    100% { box-shadow: 0 0 0 0 rgba(232,17,59,0); }
  ...
  /* nav link with animated bottom border */
  .nav-link {
    padding: 8px 16px; border-radius: var(--radius-sm); color: var(--text2);
    font-size: 0.9rem; font-weight: 500; transition: all 0.2s;
    position: relative; overflow: hidden;
  ...
  /* shine sweep on btn-primary hover */
  .btn-primary { background: var(--primary); color: #fff; }
  .btn-primary::before {
    content: ''; position: absolute; top: 0; left: -100%; width: 60%;
    height: 100%; background: rgba(255,255,255,0.2);
  ...
  /* Enhanced cards with gradient border trace on hover */
  .card {
    background: var(--surface);
    border: 1px solid var(--border);
    border-radius: var(--radius);
  ...
  /* Hero */
  .hero { padding: 80px 0 60px; position: relative; overflow: hidden; }
  /* animated mesh gradient blobs in hero */
  .hero::before {
    content: ''; position: absolute; top: -200px; right: -200px; width: 600px; height: 600px;
    background: radial-gradient(circle, rgba(232,17,59,0.10) 0%, transparent 70%);
    pointer-events: none; animation: floatYSlow 8s ease-in-out infinite;
