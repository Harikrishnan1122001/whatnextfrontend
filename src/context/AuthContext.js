...
export function AuthProvider({ children }) {
  ...
  // Rehydrate user on mount / token change
  useEffect(() => {
    if (token) {
      fetch(`${API_BASE}/auth/me`, {
        headers: { Authorization: `Bearer ${token}` },
        ...
        .then(r => r.json())
        .then(d => {
          if (d.success) setUser(d.user || d.data);
          else _clear();
        })
        .catch(_clear)
        .finally(() => setLoading(false));
  ...
  const login = async (email, password) => {
    ...
    const d = await r.json();
    if (d.success) _persist(d.token, d.user || d.data);
    return d;
  };
  ...
  // Smart fetch — handles JSON and FormData
  const authFetch = useCallback(
    async (url, opts = {}) => {
      const isFormData = opts.body instanceof FormData;
      const headers = {
