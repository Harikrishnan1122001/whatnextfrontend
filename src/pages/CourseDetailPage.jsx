...
//       if (res.success) {
//         toast('✅ Video added successfully!');
//         // FIX: backend returns { data: { video } } shape
//         onAdded(res.data?.video || res.video);
//         resetForm();
//       } else {
//         toast(res.message || 'Failed to add video', 'error');
...
//         if (d.success) {
//           // ─────────────────────────────────────────────────────────
//           // FIX: Backend sendSuccess wraps everything in d.data.
//           // getCourse returns { course, purchased } so they live at:
//           //   d.data.course   and   d.data.purchased
//           // ─────────────────────────────────────────────────────────
//           const payload = d.data || {};
//           setCourse(payload.course ?? null);
//           setPurchased(payload.purchased ?? false);
//         }
//         setLoading(false);
//       })
//       .catch(() => setLoading(false));
...
//     if (course.isFree) {
//       // Free enrolment — no payment needed, just mark purchased locally
//       // (Your backend may have a separate free-enrol endpoint; add it here if so)
//       toast('🎉 You are enrolled! Head to your dashboard.', 'success');
//       setPurchased(true);
//       return;
//     }
...
//     const res = await authFetch(`/courses/${id}/publish`, { method: 'PATCH' });
//     if (res.success) {
//       // FIX: res.data contains { isPublished }
//       const newPublished = res.data?.isPublished ?? !course.isPublished;
//       setCourse(prev => ({
//         ...prev,
//         isPublished: newPublished,
//         publishedAt: newPublished ? new Date().toISOString() : null,
//       }));
//       toast(res.message || (newPublished ? 'Course published' : 'Course unpublished'));
//     } else {
//       toast(res.message || 'Failed', 'error');
//     }
//   };
...
//   const price = course.isFree ? 0 : (course.discountPrice ?? course.price);
//   const totalDuration = course.videos?.reduce((acc, v) => acc + (v.duration || 0), 0) || 0;
//   const previewCount = course.videos?.filter(v => v.isPreview).length || 0;
...
//                         {course.videos.length} Lessons
//                         {totalDuration > 0 && (
//                           <span style={{ fontSize: '0.85rem', fontWeight: 400, color: 'var(--text2)', marginLeft: 8 }}>
//                             · {fmtDur(totalDuration)}
//                           </span>
//                         )}
//                       </h2>
//                       {!purchased && previewCount > 0 && (
//                         <span style={{ fontSize: '0.78rem', color: 'var(--text3)' }}>
...
function UploadProgress({ percent, fileName }) {
  return (
    <div style={{ marginTop: 12, padding: 12, background: 'var(--bg3)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }}>
      ...
      {percent === 100 && <p style={{ fontSize: '0.75rem', color: 'var(--green)', marginTop: 4 }}>✅ Upload complete! Processing…</p>}
    </div>
  );
}
...
function AddVideoForm({ courseId, onAdded, authFetch, toast }) {
  ...
  const [file, setFile] = useState(null);
  const fileInputRef = useRef(null);
  const [form, setForm] = useState({ title: '', videoUrl: '', description: '', duration: '', isPreview: false, buyNowTriggerSeconds: 300 });
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (tab === 'url') {
        ...
        res = await authFetch(`/courses/${courseId}/videos/url`, {
          method: 'POST',
          body: JSON.stringify({ title: form.title.trim(), videoUrl: form.videoUrl.trim(), description: form.description, duration: Number(form.duration) || 0, isPreview: form.isPreview, buyNowTriggerSeconds: Number(form.buyNowTriggerSeconds) || 300 }),
        });
      } else {
        if (!file) { toast('Please select a video file', 'error'); setSaving(false); return; }
        if (file.size > 500 * 1024 * 1024 && !window.confirm(`File is ${(file.size / 1024 / 1024).toFixed(0)}MB. Continue?`)) { setSaving(false); return; }
        const fd = new FormData();
        fd.append('video', file); fd.append('title', form.title.trim()); fd.append('description', form.description);
        fd.append('duration', String(Number(form.duration) || 0)); fd.append('isPreview', form.isPreview ? 'true' : 'false');
        fd.append('buyNowTriggerSeconds', String(Number(form.buyNowTriggerSeconds) || 300));
        const token = localStorage.getItem('token');
        setUploading(true);
        res = await uploadWithProgress(`${API_BASE}/courses/${courseId}/videos/upload`, fd, token);
      }
      if (res.success) { toast('✅ Video added!'); onAdded(res.data?.video || res.video); resetForm(); }
      else toast(res.message || 'Failed to add video', 'error');
    } catch (err) { console.error(err); toast('Upload failed. Check your connection.', 'error'); }
    ...
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 20, marginBottom: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        {tab === 'url' ? (
          ...
          <div style={{ gridColumn: '1/-1' }}>
            <label style={labelStyle}>Video File *</label>
            <div style={{ border: '2px dashed var(--border)', borderRadius: 'var(--radius-sm)', padding: 24, textAlign: 'center', cursor: 'pointer', background: file ? 'rgba(var(--primary-rgb),0.05)' : 'var(--bg1)', transition: 'all 0.2s' }}
              onClick={() => fileInputRef.current?.click()}
              onDragOver={e => e.preventDefault()}
              onDrop={e => { e.preventDefault(); const d = e.dataTransfer.files[0]; if (d?.type.startsWith('video/')) setFile(d); else toast('Please drop a video file', 'error'); }}>
              {file ? (
                <div>
                  <div style={{ fontSize: '2rem', marginBottom: 6 }}>🎬</div>
...
function VideoItem({ v, i, purchased, isAdmin, onDelete, onEdit }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)',
      marginBottom: 8, background: 'var(--bg1)',
      opacity: !canWatch ? 0.65 : 1, transition: 'opacity 0.2s',
    }}>
      ...
      {v.isPreview && !purchased && <span className="badge badge-green" style={{ flexShrink: 0 }}>Preview</span>}
      <span style={{ fontSize: '1rem', color: canWatch ? 'var(--primary-light)' : 'var(--text3)' }}>{canWatch ? '▶' : '🔒'}</span>
      {isAdmin && (
        <div style={{ display: 'flex', gap: 4 }}>
          <button onClick={() => onEdit(v)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--primary)', fontSize: '0.9rem', padding: '2px 6px', borderRadius: 4 }} title="Edit">✏️</button>
          <button onClick={() => onDelete(v._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontSize: '0.9rem', padding: '2px 6px', borderRadius: 4 }} title="Delete">🗑</button>
        </div>
      )}
    </div>
  );
...
export default function CourseDetailPage({ onAuth }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/courses/${id}`, {
      ...
      .then(d => {
        if (d.success) {
          // Backend: sendSuccess wraps in d.data → { course, purchased }
          const payload = d.data || {};
          setCourse(payload.course ?? null);
          setPurchased(payload.purchased ?? false);
        }
  ...
  const handleBuy = async () => {
    ...
    // ── FREE course: call the backend enrol endpoint ──────────
    if (course.isFree) {
      setBuying(true);
      try {
        const res = await authFetch(`/courses/${id}/enrol`, { method: 'POST', body: JSON.stringify({}) });
        if (res.success) {
          toast('🎉 Enrolled! You can now watch all videos.', 'success');
          setPurchased(true);
          // Re-fetch so video URLs are returned by backend
          const token = localStorage.getItem('token');
          const refreshed = await fetch(`${API_BASE}/courses/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json());
          if (refreshed.success) {
            const payload = refreshed.data || {};
            setCourse(payload.course ?? course);
            setPurchased(payload.purchased ?? true);
          }
        } else {
      ...
      openRazorpay({
        ...
        onSuccess: async (paymentData) => {
          ...
          if (verifyRes.success) {
            toast('🎉 Purchase successful! Happy learning!');
            setPurchased(true);
            // Re-fetch to get unlocked video URLs
            const token = localStorage.getItem('token');
            const refreshed = await fetch(`${API_BASE}/courses/${id}`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json());
            if (refreshed.success) {
              const payload = refreshed.data || {};
  ...
  const handleVideoUpdated = (updatedVideo) => {
    setCourse(prev => ({ ...prev, videos: prev.videos.map(v => v._id === updatedVideo._id ? updatedVideo : v) }));
    setEditingVideo(null);
  };
  ...
  const handleTogglePublish = async () => {
    ...
    if (res.success) {
      const newPublished = res.data?.isPublished ?? !course.isPublished;
      setCourse(prev => ({ ...prev, isPublished: newPublished, publishedAt: newPublished ? new Date().toISOString() : null }));
      toast(res.message || (newPublished ? 'Course published' : 'Course unpublished'));
    } else {
      toast(res.message || 'Failed', 'error');
    }
  ...
  return (
    <div className="page">
      {editingVideo && (
        <EditVideoModal video={editingVideo} courseId={id} authFetch={authFetch} toast={toast} onUpdated={handleVideoUpdated} onClose={() => setEditingVideo(null)} />
      )}
      ...
      <div className="detail-header">
        <div className="container">
          ...
          <h1 className="detail-title">{course.title}</h1>
          {course.shortDescription && <p style={{ color: 'var(--text2)', fontSize: '0.95rem', marginBottom: 12 }}>{course.shortDescription}</p>}
          <div className="detail-meta-row">
            <span>🎬 {course.videos?.length || 0} videos</span>
            {totalDuration > 0 && <span>⏱ {fmtDur(totalDuration)} total</span>}
            <span>👥 {course.totalStudents || 0} students</span>
      ...
      <div className="container">
        ...
        <div className="detail-layout">
          {/* Left column */}
          <div>
            {isAdmin && showAddVideo && <AddVideoForm courseId={id} onAdded={handleVideoAdded} authFetch={authFetch} toast={toast} />}
            ...
            {activeTab === 'content' && (
              <div>
                {/* Enrolled banner */}
                {purchased && (
                  <div style={{ background: 'rgba(31,168,85,0.1)', border: '1px solid rgba(31,168,85,0.4)', borderRadius: 'var(--radius-sm)', padding: '12px 16px', marginBottom: 20, fontSize: '0.9rem', color: 'var(--green)', display: 'flex', alignItems: 'center', gap: 8 }}>
                    ✅ You're enrolled — all videos are unlocked!
                  </div>
                )}
                {course.videos?.length > 0 ? (
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                      ...
                      {!purchased && previewCount > 0 && (
                        <span style={{ fontSize: '0.78rem', color: 'var(--text3)' }}>{previewCount} free preview{previewCount !== 1 ? 's' : ''}</span>
                      )}
                    </div>
                    <div className="video-list">
                      {course.videos.map((v, i) => (
                        <VideoItem key={v._id || i} v={v} i={i} purchased={purchased} isAdmin={isAdmin} onDelete={handleDeleteVideo} onEdit={setEditingVideo} />
                      ))}
                    </div>
                  </div>
                ) : (
              ...
              <div>
                ...
                {course.requirements?.length > 0 && (
                  <div style={{ marginBottom: 28 }}>
                    <h3 style={{ fontWeight: 700, marginBottom: 12 }}>Requirements</h3>
                    <ul style={{ paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {course.requirements.map((r, i) => <li key={i} style={{ color: 'var(--text2)', fontSize: '0.88rem' }}>{r}</li>)}
                    </ul>
                  </div>
                )}
                {course.outcomes?.length > 0 && (
                ...
                {course.tags?.length > 0 && (
                  <div style={{ marginBottom: 28 }}>
                    <h3 style={{ fontWeight: 700, marginBottom: 12 }}>Tags</h3>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {course.tags.map((tag, i) => <span key={i} style={{ padding: '4px 12px', background: 'var(--bg3)', borderRadius: 99, fontSize: '0.8rem', color: 'var(--text2)', border: '1px solid var(--border)' }}>{tag}</span>)}
                    </div>
                  </div>
                )}
                {course.instructor && (
                  <div style={{ marginBottom: 28, padding: 16, background: 'var(--bg2)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      {course.instructor.avatar
                        ...
                        : <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', color: '#fff', fontWeight: 700 }}>{course.instructor.name?.[0]?.toUpperCase() || '?'}</div>
                      }
                      <div>
                        <div style={{ fontWeight: 600 }}>{course.instructor.name}</div>
                        <div style={{ fontSize: '0.82rem', color: 'var(--text2)' }}>Course Instructor</div>
          ...
          <div className="detail-sidebar">
            ...
            <div className="sidebar-features" style={{ marginTop: 16 }}>
              {[
                ...
                <div key={text} className="sidebar-feature"><span className="icon">{icon}</span><span>{text}</span></div>
              ))}
            </div>
              ...
              <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                ...
                {course.notes.map(n => (
                  <div key={n._id} style={{ padding: '10px 12px', background: 'var(--bg1)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', marginBottom: 8 }}>
                    <div style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 4 }}>{n.title}</div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '0.8rem', color: n.isFree ? 'var(--green)' : 'var(--text2)' }}>{n.isFree ? 'Free' : `₹${n.effectivePrice ?? n.price}`}</span>
                      <button className="btn btn-primary" style={{ padding: '3px 10px', fontSize: '0.75rem' }} onClick={() => navigate(`/notes/${n._id}`)}>View</button>
                    </div>
                  </div>
                ))}
              </div>
