...
export default function CourseDetailPage({ onAuth }) {
  ...
  // ── Fetch course (also reusable for post-purchase refresh) ───
  const loadCourse = async () => {
    ...
    try {
      ...
      if (res.success) {
        // Backend: sendSuccess wraps in res.data → { course, purchased }
        const payload = res.data || {};
        setCourse(payload.course ?? null);
        setPurchased(payload.purchased ?? false);
      }
    } catch {
      // swallow — handled by loading state below
    }
  };
  ...
  const handleBuy = async () => {
    ...
    if (course.isFree) {
      ...
      try {
        ...
        if (res.success) {
          toast('🎉 Enrolled! You can now watch all videos.', 'success');
          await loadCourse();                  // pulls fresh `purchased` + unlocked video URLs
          if (refreshUser) await refreshUser(); // keeps Dashboard/user object in sync
        } else {
          toast(res.message || 'Enrolment failed', 'error');
        }
      } catch {
