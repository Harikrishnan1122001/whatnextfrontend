...
// Watermarks the viewer's identity, blocks easy capture shortcuts, and pauses
// when the tab loses focus. Nothing can block OS-level screen recording 100% —
// this just removes the easy paths and makes leaked footage traceable.
function ProtectedVideoPlayer({ video, courseId, user, authFetch, toast, onClose }) {
  const videoRef = useRef(null);
  const [wm, setWm] = useState({ top: '10%', left: '10%' });
  const [streamUrl, setStreamUrl] = useState(null);
  const [loadingUrl, setLoadingUrl] = useState(true);

  // Fetch a short-lived signed URL instead of using the raw videoUrl forever
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await authFetch(`/courses/${courseId}/videos/${video._id}/stream-token`);
        if (!cancelled) {
          if (res.success) setStreamUrl(`${res.data.videoUrl}${res.data.videoUrl.includes('?') ? '&' : '?'}token=${res.data.token}`);
          else { toast(res.message || 'Could not load video', 'error'); onClose(); }
        }
      } catch {
        if (!cancelled) { toast('Could not load video', 'error'); onClose(); }
      } finally {
        if (!cancelled) setLoadingUrl(false);
      }
    })();
    return () => { cancelled = true; };
  }, [video._id, courseId]); // eslint-disable-line react-hooks/exhaustive-deps

  // Move the watermark every few seconds so it can't be cropped out of a recording
  useEffect(() => {
    const id = setInterval(() => {
      setWm({ top: `${10 + Math.random() * 70}%`, left: `${10 + Math.random() * 70}%` });
    }, 4000);
    return () => clearInterval(id);
  }, []);

  // Pause on tab-switch / window blur — deters screen-share/recording tools that need focus
  useEffect(() => {
    const pause = () => { if (videoRef.current && !videoRef.current.paused) videoRef.current.pause(); };
  ...
  return (
    ...
    >
      <div
        style={{ background: '#000', borderRadius: 'var(--radius)', overflow: 'hidden', width: '100%', maxWidth: 920, border: '1px solid var(--border)' }}
        onClick={(e) => e.stopPropagation()}
        onContextMenu={blockContext}
      >
        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: 8, background: 'var(--bg2)' }}>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.1rem', color: 'var(--text2)' }}>✕</button>
        </div>
        ...
        <div style={{ padding: 16, background: 'var(--bg2)' }}>
          <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text1)' }}>{video.title}</div>
          {video.description && <div style={{ fontSize: '0.85rem', color: 'var(--text2)', marginTop: 4 }}>{video.description}</div>}
        </div>
      </div>
    </div>
  );
...
function VideoItem({ v, i, purchased, isAdmin, onDelete, onEdit, onPlay }) {
  ...
  return (
    <div
      ...
      onMouseEnter={e => { if (canWatch) e.currentTarget.style.transform = 'translateX(4px)'; }}
